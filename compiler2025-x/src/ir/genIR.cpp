...
// 隐式转换功能：检查接受的值的类型和目标变量的类型，不一致时进行转换
void GenIR::checkInitType() const {
    if(curType == INT32_T) {
        if(dynamic_cast<ConstantFloat*>(recentVal)) { // 常量直接进行截断或者补全即可
            auto temp = dynamic_cast<ConstantFloat*>(recentVal);
            recentVal = CONST_INT((int)temp->value_);
        } else if(recentVal->type_->tid_ == Type::FloatTyID) { // 变量需插入转换指令在运行时进行替换
...
// 编译单元的访问函数(Compile Unit)：CompUnit也是SysY语言文法的开始符号
void GenIR::visit(CompUnitAST& ast) {
    // cout << "CompUnitAST: list_size: " << ast.declDefList.size() << endl;
    for(const auto& def : ast.declDefList) {
        def->accept(*this);
    }
}

// 声明或函数定义(Declaraton of Function Definition)
void GenIR::visit(DeclDefAST& ast) {
    // cout << "DeclDefAST" << endl;
    if(ast.Decl != nullptr) {
...
// 声明(Declaration)：变量及常量的声明
void GenIR::visit(DeclAST& ast) {
    // cout << "DeclAST: " << (ast.isConst ? "const " : "") << (ast.bType == TYPE_INT ? "int" : "float") << endl;
    // 记录当前声明是否为常量以及其类型
    isConst = ast.isConst;
    curType = ast.bType == TYPE_INT ? INT32_T : FLOAT_T; // SysY2022语言中只有int(1or32bits)和float(32bits)两种类型 
    for(auto& def : ast.defList) {
        def->accept(*this);
    }
}

// 变量or常量的定义(Definition)
void GenIR::visit(DefAST& ast) {
    string varName =* ast.id;
    // cout << "DefAST: " << varName << " ";
    if(scope.in_global()) { // 全局变量或常量
        // cout << "global ";
        if(ast.arrays.empty()) { // 不是数组，即全局量
            // cout << "variable" << endl;
            if(ast.initVal == nullptr) { // 无初始化
                if(isConst) {
                    cout << "no initVal when define const!" << endl; // 无初始化全局常量报错
                }
                GlobalVariable* var; // 此时一定为全局变量
                if(curType == INT32_T) {
                    var = new GlobalVariable(varName, module.get(), curType, false, CONST_INT(0));
                } else {
                    var = new GlobalVariable(varName, module.get(), curType, false, CONST_FLOAT(0));
                }
                scope.push(varName, var);
            } else { // 有初始化
                useConst = true;
                ast.initVal->accept(*this); // 计算初始值
                useConst = false;
                checkInitType();
                if(isConst) { // 全局常量，直接插入符号表，无需GlobalVariable
                    scope.push(varName, recentVal);
                } else { // 全局变量
            ...
            useConst = false;
            vector<ArrayType*> arrayTys(dimensions.size()); // 数组类型：{[2*  [3*  [4*  i32]]], [3*  [4*  i32]], [4*  i32]}
...
void GenIR::mergeElements(vector<int>& dimensions, vector<ArrayType*>& arrayTys, int up, int dimAdd,
...
void GenIR::finalMerge(vector<int>& dimensions, vector<ArrayType*>& arrayTys, int up,
                        vector<Constant*>& elements, vector<int>& elementsCnts) const {
...
void GenIR::visit(FuncDefAST& ast) {
    // cout << "FuncDefAST: " <<* ast.id << ", return type: " << ast.funcType << endl;
    isNewFunc = true;
    params.clear();
    paramNames.clear();
    Type* retType;
...
void GenIR::visit(FuncFParamAST& ast) {
    // cout << "FuncFParamAST: " <<* ast.id << ", type: " << ast.bType << endl;
    // 获取参数类型
    Type* paramType;
...
void GenIR::visit(BlockAST& ast) {
    // cout << "BlockAST: blockItemList size: " << ast.blockItemList.size() << endl;
    if(isNewFunc) { // 如果是一个新的函数，则不用再进入一个新的作用域，因为在FuncDef中已经进入了一个新的作用域
        isNewFunc = false;
...
void GenIR::visit(StmtAST& ast) {
    switch(ast.sType) {
        case SEMI:
            break;
        case ASS: {
            requireLVal = true; // Init lVal state
            ast.lVal->accept(*this); // Visit lVal
            auto lVal = recentVal; // Get lVal
            auto lValType = static_cast<PointerType*>(lVal->type_)->contained_;
            ast.exp->accept(*this); // Visit expression
            auto rVal = recentVal;
            if(lValType != recentVal->type_) { // if lVal.type != rVal.type Forge a cast
                if(lValType == FLOAT_T) {
            ...
            builder->create_store(rVal, lVal); // Create a store primitive
            break;
        }
        case EXP:
            is_single_exp = true;
...
void GenIR::visit(NumberAST& ast) {
    // cout << "NumberAST: ";
    if(ast.isInt) {
        // cout << "int: " << ast.intval << endl;
        recentVal = CONST_INT(ast.intval);
    } else {
        // cout << "float: " << ast.floatval << endl;

...
RiscvOperand *RegAlloca::findReg(Value *val, RiscvBasicBlock *bb,
  ...
  if (specified != nullptr)
   {
      bool reallyLoad = setPositionReg2(val, specified, bb, instr);
      if (load == true)
      {
          load = reallyLoad; // 如果原始寄存器对应的value即为目标value，就没有Load的必要了
      }
   }
    ...
    FloatRegID = 32;
    while (!found) {
      // std::cout << "no found" << IntRegID << " " << FloatRegID << std::endl;
    ...
    // std::cout << "Have already find" << std::endl;
    ...
    if (mem_addr != nullptr) {
      ...
      if (cval != nullptr)
        ...
        if (dynamic_cast<BinaryInst*>(val))
			...
			{
				std::cout << "StoreInst" << std::endl;
			}
			else if (dynamic_cast<LoadInst*>(val))
			{
				std::cout << "LoadInst" << std::endl;
				auto op0 = dynamic_cast<LoadInst*>(val)->operands_[0];
				if(dynamic_cast<AllocaInst*>(op0) != nullptr)
					std::cout << "whatAlloca " << op0 << std::endl;
			}
			else if (dynamic_cast<AllocaInst*>(val)) // Alloca操作应该保持正常(在初始时刻的Alloca操作已经处理结束)
			{
				std::cout << "AllocaInst" << std::endl;
			}
			else if (dynamic_cast<ZextInst*>(val))
			{
				std::cout << "ZextInst" << std::endl;
			}
			else if (dynamic_cast<FpToSiInst*>(val))
			{
				std::cout << "FptoSiInst" << std::endl;
			}
			else if (dynamic_cast<SiToFpInst*>(val))
			{
				std::cout << "SiToFpInst" << std::endl;
			}
			else if (dynamic_cast<Bitcast*>(val))
			{
				std::cout << "BitCastInst" << std::endl;
			}
			else if (dynamic_cast<PhiInst*>(val))
			{
				std::cout << "PhiInst" << std::endl;
			}
  ...
  // std::cout << "I find and I out" << std::endl;
  return current_reg;
}
...
RiscvOperand *RegAlloca::findMem(Value *val, RiscvBasicBlock *bb,
  ...
  if (isPointer && !isAlloca && !direct) {
    ...
    if(curReg.find(val) != curReg.end())
    	return new RiscvIntPhiReg(curReg[val]->print());
...
bool RegAlloca::setPositionReg2(Value* val, RiscvOperand* riscvReg,
    RiscvBasicBlock* bb, RiscvInstr* instr) {
    bool reallyLoad = true;
    val = this->DSU_for_Variable.query(val);
    Value* old_val = getRegPosition(riscvReg);
    RiscvOperand* old_reg = getPositionReg(val);
    if (old_val != nullptr && old_val != val)
...
void RegAlloca::onlyErase(RiscvOperand* riscvReg, RiscvBasicBlock* bb,
    RiscvInstr* instr)
{
    Value* value = getRegPosition(riscvReg);
    if (value == nullptr)
    ...
    if(regPos.find(riscvReg) != regPos.end())
       regPos.erase(riscvReg);
    if(regFindTimeStamp.find(riscvReg) != regFindTimeStamp.end())
    	regFindTimeStamp.erase(riscvReg);
    if(curReg.find(value) != curReg.end())
        curReg.erase(value);
...
void RegAlloca::writeback_all(RiscvBasicBlock *bb, RiscvInstr *instr) {
  ...
  for (auto r : regs_to_writeback)
    {
    // std::cout << "r->name = << " << r->print() << std::endl;

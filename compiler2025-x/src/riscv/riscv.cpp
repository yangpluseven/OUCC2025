...
RiscvFunction::RiscvFunction(std::string name, int num_args, OpTy Ty) // 返回值，无返回使用void类型
...
RiscvFunction *createSyslibFunc(Function *foo) {
    if (foo->name_ == "__aeabi_memclr4") {
        ...
        // 循环块
        // 默认clear为全0
        bb2->addInstrBack(new StoreRiscvInst( new Type(Type::TypeID::IntegerTyID), getRegOperand("zero"), new RiscvIntPhiReg(NamefindReg("t5")), bb2));

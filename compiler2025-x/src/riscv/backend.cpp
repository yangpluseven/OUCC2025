...
BinaryRiscvInst *RiscvBuilder::createBinaryInstr(RegAlloca *regAlloca,
                                                 ...
                                                 RiscvBasicBlock *rbb) {
  assert(toRiscvOp.find(binaryInstr->op_id_) != toRiscvOp.end());
  ...
  BinaryRiscvInst *instr = new BinaryRiscvInst(
      ...
      // if(regAlloca->findReg(binaryInstr->operands_[0], rbb, nullptr, 1, 0) == regAlloca->findReg(binaryInstr, rbb, nullptr, 1, 0))
...
UnaryRiscvInst *RiscvBuilder::createUnaryInstr(RegAlloca *regAlloca,
                                               ...
                                               RiscvBasicBlock *rbb) {
  assert(toRiscvOp.find(unaryInstr->op_id_) != toRiscvOp.end());
...
bool judgeParams(Function* foo, Value* val)
{
    for (auto param : foo->arguments_)
    {
        if (param == val)
            return true;
    }
...
//修这个bug
void createPhiStoreInstr(RegAlloca* regAlloca, BasicBlock* bb)
{ 
         //用来记录前面扫描过的phi语句
    std::set<Instruction*> phiSet;
    // 用于记录每个phi指令的每个操作数对应的block集合 
    std::unordered_map<Instruction*, std::unordered_set<BasicBlock*>> phiIncomingBlocks;
    std::unordered_map<Instruction*, RiscvInstr*> phiIncominginst;

    // 检查phiInstr是否被分配对应的内存空间
    for (auto instr : bb->instr_list_)
    {
        ...
        phiSet.insert(instr);
        //对于phi语句
        auto memo = regAlloca->findMem(instr);
        if (memo == nullptr)
        {
            std::cout << "try to store phiInstr, but memo is nullptr, so it may be unused tmpVar" << std::endl;
            return;
        }

        for (int i = 0; i < instr->operands_.size(); i += 2){
          phiIncomingBlocks[instr].insert(dynamic_cast<BasicBlock*>(instr->operands_[i + 1]));
          //对于操作数是常数的情况，不需要特殊处理
          if (instr->operands_[i]->is_constant()){

            auto aimedBasicBlock = dynamic_cast<BasicBlock*>(instr->operands_[i + 1]);
            auto aimedRiscvBasicBlock = rbbLabel[aimedBasicBlock];
            auto testConstantInt = dynamic_cast<ConstantInt*>(instr->operands_[i]);

            std::vector<RiscvInstr*> ans;
            if (testConstantInt != nullptr)
            {
                auto regPos = getRegOperand("t0"); // First：获取寄存器操作数
                ans.push_back(new MoveRiscvInst(regPos, new RiscvConst(testConstantInt->value_), aimedRiscvBasicBlock));
                
                phiIncominginst[instr] = ans.back();
                ans.push_back(new StoreRiscvInst(
                    instr->type_, // Store的类型
                    regPos, // 源操作数（寄存器存放）
                    memo,// 目的操作数（目标内存）
                    aimedRiscvBasicBlock));
            }
            else
            {
                int sz = aimedRiscvBasicBlock->instruction.size();
                auto regPos = regAlloca->findReg(instr->operands_[i], aimedRiscvBasicBlock, nullptr, 1);
                assert(sz + 2 == aimedRiscvBasicBlock->instruction.size());
                for (int t = 2; t >= 1; t--)
                {
                    ans.push_back(aimedRiscvBasicBlock->instruction[aimedRiscvBasicBlock->instruction.size() - t]);
                }
                for (int t = 1; t <= 2; t++)
                {
                    aimedRiscvBasicBlock->deleteInstr(aimedRiscvBasicBlock->instruction.back());
                }
                ans.push_back(new StoreRiscvInst(instr->operands_[i]->type_, regPos, memo, aimedRiscvBasicBlock));
            }

            // 将指令插入基本块后端
            for (auto newInstr : ans)
            {
                aimedRiscvBasicBlock->addInstrBeforeTerminator(newInstr);
            }
          }
          else if(dynamic_cast<ValUndef*>(instr->operands_[i])) // 这种情况直接返回
          {
            continue;
          }  
          else{ 
            ...
            auto aimedBasicBlock = dynamic_cast<BasicBlock*>(instr->operands_[i + 1]);
            auto aimedRiscvBasicBlock = rbbLabel[aimedBasicBlock];
            bool isInt = instr->type_->tid_ == Type::IntegerTyID;
            std::vector<RiscvInstr*> ans;
            if (isInt != false)
            {
                auto regPos = getRegOperand("t0");
                ans.push_back(new LoadRiscvInst(instr->type_, regPos, regAlloca->findMem(instr->operands_[i], aimedRiscvBasicBlock, nullptr, false), aimedRiscvBasicBlock));
                phiIncominginst[instr] = ans.back();
                ans.push_back(new StoreRiscvInst(
                    instr->type_, // Store的类型
                    regPos, // 源操作数（寄存器存放）
            ...
            {
              auto regPos = getRegOperand("fs1");

              auto srcMem = regAlloca->findMem(instr->operands_[i], aimedRiscvBasicBlock, nullptr, false);
              if (!srcMem) {
                  std::cerr << "Error: no mem slot for " << instr->operands_[i]->name_ << std::endl;
                  continue;
              }
              
              // Type * newtmp = new Type(Type::FloatTyID);
              // Load: 使用 fld (浮点 load)
              ans.push_back(new LoadRiscvInst(instr->type_, regPos, srcMem, aimedRiscvBasicBlock));
          
              // Store: 使用 fsd (浮点 store)
              ans.push_back(new StoreRiscvInst(instr->type_, regPos, memo, aimedRiscvBasicBlock));
            }

            //如果操作数是变量 需要判断operand是否是之前的phi 且是公共前驱
            BasicBlock * preb = dynamic_cast<BasicBlock*>(instr->operands_[i + 1]);
            //如果operand不是之前的phi 正常处理
            if (phiSet.find(static_cast<Instruction*>(instr->operands_[i])) == phiSet.end() || phiIncomingBlocks[static_cast<Instruction*>(instr->operands_[i])].find(preb) == phiIncomingBlocks[static_cast<Instruction*>(instr->operands_[i])].end()) {
              for (auto newInstr : ans){
                aimedRiscvBasicBlock->addInstrBeforeTerminator(newInstr);
              }
            }
            else{ //需要对前驱的phi语句进行调整 不是插在最后了，要插在这个前面
                //for(auto inst:ans) std::cout << inst->print() << std::endl;

                for(auto oldinstr: aimedRiscvBasicBlock->instruction){
                  if(oldinstr == phiIncominginst[static_cast<Instruction*>(instr->operands_[i])]){
                    // for(auto inst:aimedRiscvBasicBlock->instruction)
                    //   std::cout << inst->print() << std::endl;
                    //   std::cout  << std::endl << std::endl;
                    for(auto newInstr : ans){
                      aimedRiscvBasicBlock->addInstrBefore(newInstr,oldinstr);
                      // for(auto inst:aimedRiscvBasicBlock->instruction)
                      // std::cout << inst->print() << std::endl;
                      // std::cout  << std::endl << std::endl;
                    }
...
ICmpRiscvInstr *RiscvBuilder::createICMPInstr(RegAlloca *regAlloca,
  ...
  ICmpRiscvInstr *instr = new ICmpRiscvInstr(
      ...
      rbb);
      if(regAlloca->findReg(icmpInstr->operands_[0], rbb, nullptr, 0)->print() == "a0")
      {
      	std::cout << "why" << std::endl;
      }
      std::cout << "dadadadadadada " << regAlloca->findReg(icmpInstr->operands_[0], rbb, nullptr, 0)->print() << std::endl;
  return instr;
}
...
ReturnRiscvInst *RiscvBuilder::createRetInstr(RegAlloca *regAlloca,
  ...
  if (returnInstr->num_ops_ > 0) {
    ...
    //    getOperand("a0"), regAlloca->findMem(operand, rbb, nullptr), rbb));
    // }
    
    regAlloca->onlyErase(regAlloca->findReg(operand, rbb, nullptr, 1, 0), rbb, nullptr);
  }
...
BranchRiscvInstr *RiscvBuilder::createBrInstr(RegAlloca *regAlloca,
  ...
  if (brInstr->num_ops_ == 1) {
    ...
    instr = new BranchRiscvInstr(
        ...
        createRiscvBasicBlock(static_cast<BasicBlock *>(brInstr->operands_[2])),
        rbb);
        // regAlloca->onlyErase(regAlloca->findReg(brInstr->operands_[0], rbb, nullptr, 1, 0), rbb, nullptr);
  }
  return instr;
}
...
RiscvInstr *RiscvBuilder::solveGetElementPtr(RegAlloca *regAlloca,
    ...
    if (instr->hasBasePtr)
    {
    	// std::cout << "dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd" << std::endl;
        GetElementPtrInst* basePtr = instr->basePtr;
        int offset = instr->offset;
        // RiscvOperand* dest = getRegOperand("t2");
        rbb->addInstrBack(new BinaryRiscvInst(RiscvInstr::InstrType::ADDI, regAlloca->findReg(basePtr, rbb, nullptr, 1), new RiscvConst(offset), regAlloca->findReg(instr, rbb, nullptr, 1, 0), rbb));
        // rbb->addInstrBack(new StoreRiscvInst(instr->type_, regAlloca->findReg(instr, rbb, nullptr, 1, 0), regAlloca->findMem(instr), rbb));
        return nullptr;
    }
  ...
  else if (auto oi = dynamic_cast<Argument*>(op0)) // 【a little difference：有很多指令可以直接使用参数】
  {
      rbb->addInstrBack(new MoveRiscvInst(
        dest, regAlloca->findReg(op0, rbb, nullptr, 1, 1), rbb));
  }
...
RiscvBasicBlock *RiscvBuilder::transferRiscvBasicBlock(BasicBlock *bb,
  ...
  for (Instruction *instr : bb->instr_list_) {
	...
	std::string calleeFooName = static_cast<Function*>(curInstr->operands_.back())->name_;
	
	// 需要先保护a0、a1、a2寄存器和fa0、fa1、fa2寄存器
	/*if (putSysFunc.find(calleeFooName) != putSysFunc.end())
	{
	    foo->regAlloca->writeback(getRegOperand("a2"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("a1"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("a0"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("fa2"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("fa1"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("fa0"), rbb, nullptr);
	}
	
	else if(getSysFunc.find(calleeFooName) != getSysFunc.end())
	{
	   foo->regAlloca->writeback(getRegOperand("a2"), rbb, nullptr);
	   foo->regAlloca->writeback(getRegOperand("a1"), rbb, nullptr);
	   foo->regAlloca->writeback(getRegOperand("a0"), rbb, nullptr);
	...
	{
		// foo->regAlloca->writeback_all(rbb);
	    foo->regAlloca->writeback(getRegOperand("a7"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("a6"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("fa7"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("fa6"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("a5"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("a4"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("a3"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("fa5"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("fa4"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("fa3"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("a2"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("a1"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("a0"), rbb, nullptr);
	    foo->regAlloca->writeback(getRegOperand("fa2"), rbb, nullptr);
      ...
      // [a little difference]： a0寄存器会被修改，需要剔除a0寄存器的映射关系
      /*
      if (putSysFunc.find(calleeFooName) != putSysFunc.end())
	{
	    // erase关系
	    foo->regAlloca->onlyErase(getRegOperand("a2"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("a1"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("a0"), rbb, nullptr);
	}
      */
      
      {
	    foo->regAlloca->onlyErase(getRegOperand("a7"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("a6"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("fa7"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("fa6"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("a5"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("a4"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("a3"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("fa5"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("fa4"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("fa3"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("a2"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("a1"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("a0"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("fa2"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("fa1"), rbb, nullptr);
	    foo->regAlloca->onlyErase(getRegOperand("fa0"), rbb, nullptr);
	}
       /*intRegCount = 0, floatRegCount = 0;
       for (int i = 0; i < curInstr->operands_.size() - 1; i++) {
        std::string name = "";
        auto operand = curInstr->operands_[i];
        if (operand->type_->tid_ != Type::FloatTyID) {
        ...
        if (name.empty()) {
          ...
          foo->regAlloca->onlyErase(getRegOperand(name), rbb, nullptr);
        }
      }*/
      break;
    }
  ...
  if (!brFound) {
    // std::cout << "br Not Found-------------------------------------------------------" << std::endl;
    foo->regAlloca->writeback_all(rbb);
  }
  return rbb;
}
...
std::string RiscvBuilder::buildRISCV(Module *m) {
  ...
  for (Function *foo : m->function_list_) {
    Instruction* RetInstruction = nullptr;
    // std::cout << "foo->name = " << foo->name_ << std::endl;
    ...
    for (Value *arg : foo->arguments_)
      ...
      for (Instruction *instr : bb->instr_list_)
        if (instr->op_id_ == Instruction::OpID::PHI) {
          for (int i = 0; i < instr->operands_.size(); i += 2) {
	       // 【a little difference】，确保只会和非常数产生效应
	       // if (instr->operands_[i]->is_constant() == false  && judgeParams(foo, instr->operands_[i]) == false)
	       // 事实证明，好像基本上都没有机会merge
	    ...
	    // if(false) 
	    // {
	    //   rfoo->regAlloca->DSU_for_Variable.merge(instr->operands_[i], static_cast<Value*>(instr));
	    // }
          } 
   ...
   //////////////////////////////////////////////////////////////////////////// 
     for (auto bb : foo->basic_blocks_){
        createPhiStoreInstr(rfoo->regAlloca,bb);
     }

    // for (auto bb : foo->basic_blocks_)
    // for (auto instr : bb->instr_list_)
    //    if (dynamic_cast<PhiInst*>(instr))
    //        createPhiStoreInstr(rfoo->regAlloca, dynamic_cast<PhiInst*>(instr));
   ///////////////////////////////////////////////////////////////////////// 

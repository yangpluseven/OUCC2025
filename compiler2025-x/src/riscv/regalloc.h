...
public:
  ...
  void setPositionReg(Value *val, RiscvOperand *riscvReg, RiscvBasicBlock *bb,
   ...
   bool setPositionReg2(Value* val, RiscvOperand* riscvReg, RiscvBasicBlock* bb, RiscvInstr* instr);
  /**
   * 将 Value 与指定的寄存器 rs 相关联。若寄存器内已有值，则将值写回。
   * @param val 需要关联的 Value
   * @param riscvReg 被关联的寄存器 rs
  ...
  void onlyErase(RiscvOperand* riscvReg, RiscvBasicBlock* bb, RiscvInstr* instr = nullptr);
  /**

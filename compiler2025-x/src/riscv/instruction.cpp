...
void RiscvBasicBlock::addInstrBeforeTerminator(RiscvInstr* instr)
{
      // std::cout << "Unblievable" << std::endl;
      if (instr == nullptr)
          return;
      ...
      else
      {
          ...
          RiscvInstr* lastInstruction = instruction.back();
          if (lastInstruction->type_ == RiscvInstr::RET)
          {
              addInstrBefore(instr, lastInstruction);
          }
          else if(lastInstruction->type_ == RiscvInstr::BGT)
          {
          	// 判断指令操作数0
          	auto op0 = lastInstruction->operand_[0];
          	if(op0 == nullptr)
          	{	
          		addInstrBefore(instr, lastInstruction);
          	}
          	else
          	{
          		...
          		RiscvInstr* preInstruction = instruction[instruction.size() - 2];
          		addInstrBefore(instr, preInstruction);
          	}
          }
          else
          {
              // std::cout << "heeeeeerrrrr" << lastInstruction->type_ << std::endl;
              addInstrBefore(instr, lastInstruction);
          }
      }
}
